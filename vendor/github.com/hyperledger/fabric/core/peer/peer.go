...
func getCurrConfigBlockFromLedger(ledger ledger.PeerLedger) (*common.Block, error) {
	...
	if configBlock, err := ledger.ReadConfigBlockFromSpecFile(); err == nil && configBlock != nil {
		if configBlock.Header.Number == configBlockIndex {
			peerLogger.Debugf("Got config block from spec file")
			return configBlock, nil
		} else {
			peerLogger.Debugf("The last config index is different from the spec file.")
		}
	} else {
		peerLogger.Debugf("Failed to read config block from file:[%s]", err)
	}
