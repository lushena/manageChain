...
func (goPlatform *Platform) GenerateDockerBuild(cds *pb.ChaincodeDeploymentSpec, tw *tar.Writer) error {
	...
	err = util.DockerBuild(util.DockerBuildOptions{
		...
		Cmd:          fmt.Sprintf("LD_LIBRARY_PATH=/usr/local/lib CGO_LDFLAGS=\"-I/usr/local/include -L/usr/local/lib\" GOPATH=/chaincode/input:$GOPATH go build -ldflags  \"%s\" -o /chaincode/output/chaincode %s", ldflags, pkgname),
