...
func (vm *DockerVM) createContainer(ctxt context.Context, client dockerClient,
	...
	var chaincodeLogLevel string
	var loginfo chaincodeLogInfo
	...
	if err == nil {
		...
		// host-src
		source = config.GetString("logging.chaincodeLogPath")
		chaincodeLogLevel = config.GetString("logging.chaincodeLogLevel")

		loginfo.isOpenGglog = config.GetString("logging.isOpenGglog")
		loginfo.maxlinesPerFile = config.GetString("logging.maxlinesPerFile")
		loginfo.maxsizePerFile = config.GetString("logging.maxsizePerFile")
		loginfo.maxTotalSize = config.GetString("logging.maxTotalSize")
		loginfo.maxdays = config.GetString("logging.maxdays")
		loginfo.isAutoDelete = config.GetString("logging.isautodelete")
		loginfo.daily = config.GetString("logging.daily")
		loginfo.rotate = config.GetString("logging.rotate")
	}

	if destination == "" || len(destination) > logpathLenMax {
		dockerLogger.Warningf("log config args err, logpath:%s, use default.\n", destination)
		destination = "/var/fabric_logs/"
	}
	if source == "" || len(source) > lognameLenMax {
		dockerLogger.Warningf("log config args err, logname:%s, use default.\n", source)
		source = "/var/chaincode_logs/"
	}

	source = source + "/" + containerID

	if len(source) > logmapNameLenMax {
		dockerLogger.Warningf("chaincode log path length too long, source:%s, using hash value.\n", source)

		containerSlice := strings.Split(containerID, "-")
		for i, _ := range containerSlice {
			ccversion = containerSlice[i]
		}

		sourceFrond := source[:logmapNameLenMax-1]
		sourceEnd := md5Hash(string(source[logmapNameLenMax:]))
		source = sourceFrond + "-" + string(sourceEnd[0:8]) + "-" + ccversion

		dockerLogger.Debugf("chaincode log path path:%s\n", source)
	}
	...
	chaincodeLogLevelEnv := "CHAINCODE_LOG_LEVEL=" + chaincodeLogLevel
	logDestinationEnv := "CHAINCODE_LOG_DESTINATION=" + destination

	maxlinesEnv := "CHAINCODE_LOG_MAXLINES=" + loginfo.maxlinesPerFile
	maxsizeEnv := "CHAINCODE_LOG_MAXSIZE=" + loginfo.maxsizePerFile
	maxTotalSizeEnv := "CHAINCODE_LOG_MAXTOTALSIZE=" + loginfo.maxTotalSize
	isAutoDeleteEnv := "CHAINCODE_LOG_ISAUTODELETE=" + loginfo.isAutoDelete
	dailyEnv := "CHAINCODE_LOG_DAILY=" + loginfo.daily
	rotateEnv := "CHAINCODE_LOG_ROTATE=" + loginfo.rotate
	maxdaysEnv := "CHAINCODE_LOG_MAXDAYS=" + loginfo.maxdays
	isOpenGglogEnv := "CHAINCODE_LOG_ISOPENGGLOG=" + loginfo.isOpenGglog

	env = append(env, []string{chaincodeLogLevelEnv, logDestinationEnv, maxlinesEnv, maxsizeEnv, maxTotalSizeEnv, isAutoDeleteEnv, dailyEnv, rotateEnv, maxdaysEnv, isOpenGglogEnv}...)
