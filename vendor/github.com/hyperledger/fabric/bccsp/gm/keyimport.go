...
func (ki *sm4ImportKeyOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	keyRaw, ok := raw.([]byte)
	if !ok {
		return nil, errors.New("Invalid raw material. Expected byte array.")
	}
	...
	if len(keyRaw) != 16 {
		return nil, fmt.Errorf("Invalid Key Length [%d]. Must be 32 bytes", len(keyRaw))
	}
