...
func (store *fsBlockStore) DataDump(dumpReason int) error {
	...
	return store.fileMgr.tryDataDump(dumpReason)
}

func (store *fsBlockStore) WriteConfigBlockToSpecFile(block *common.Block) error {
	if !store.conf.dumpConf.Enabled {
		return nil
	}
	return store.fileMgr.writeConfigBlock(block)
}

func (store *fsBlockStore) ReadConfigBlockFromSpecFile() (*common.Block, error) {
	if !store.conf.dumpConf.Enabled {
		return nil, fmt.Errorf("DataDump is not enabled, cannot read config block from spec file")
	}
	return store.fileMgr.readConfigBlock()
}
