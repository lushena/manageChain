...
const (
	...
	blockFileFormatString = "blockfile_%06d"
	tarFileFormatString   = "blockfile_%06d_%06d.tar"
...
type blockfileMgr struct {
	...
	bcInfo                   atomic.Value
	rwMutexs                 []*sync.RWMutex
	dumpMutex                *sync.Mutex
	oldestFileChunkSuffixNum int
}
...
func syncOldestFileNum(rootDir string) int {
	oldestFileNum := 0
	...
	for _, info := range infos {
		var number int
		if _, err := fmt.Sscanf(info.Name(), blockFileFormatString, &number); err != nil {
			return 0
		} else {
			oldestFileNum = number
			break
		}
	}
	return oldestFileNum
}
...
func (mgr *blockfileMgr) moveToNextFile() {
	cpInfo := &checkpointInfo{
		...
		fileEndBlockNums:         mgr.cpInfo.fileEndBlockNums,
	}
	cpInfo.fileEndBlockNums = append(cpInfo.fileEndBlockNums, cpInfo.lastBlockNumber)
...
func (mgr *blockfileMgr) addBlock(block *common.Block) error {
	...
	if currentOffset+totalBytesToAppend > mgr.conf.maxBlockfileSize {
		...
		go func() {
			if mgr.conf.dumpConf.Enabled {
				if err = mgr.tryDataDump(datadump.DumpForBlockFileLimit); err != nil {
					logger.Errorf("Failed to data dump for [%s]", err)
				}
			}
		}()
	}
	...
	newCPInfo := &checkpointInfo{
		...
		fileEndBlockNums:         currentCPInfo.fileEndBlockNums,
	}
...
func (mgr *blockfileMgr) fetchBlock(lp *fileLocPointer) (*common.Block, error) {
	blockBytes, err := mgr.fetchBlockBytesWithRetry(lp)
	if err != nil {
		return nil, err
	}
...
func (mgr *blockfileMgr) fetchTransactionEnvelope(lp *fileLocPointer) (*common.Envelope, error) {
	...
	if txEnvelopeBytes, err = mgr.fetchRawBytesWithRetry(lp); err != nil {
		return nil, err
	}
...
func (mgr *blockfileMgr) fetchBlockBytesWithRetry(lp *fileLocPointer) ([]byte, error) {
	var result []byte
	var err error
	retryTimes := 1
	if mgr.conf.dumpConf.Enabled && mgr.conf.dumpConf.LoadRetryTimes > 1 {
		retryTimes = mgr.conf.dumpConf.LoadRetryTimes
	}
	for i := 0; i < retryTimes; i++ {
		if result, err = mgr.fetchBlockBytes(lp); err == nil {
			return result, err
		}
	}
	return nil, fmt.Errorf("FetchBlockBytesError: [%s]", err)
}

func (mgr *blockfileMgr) fetchBlockBytes(lp *fileLocPointer) ([]byte, error) {
	if mgr.conf.dumpConf.Enabled && lp.fileSuffixNum < mgr.oldestFileChunkSuffixNum {
		logger.Debugf("the file has been dumped fileSuffixNum :[%d], oldestFileChunkSuffixNum: [%d]", lp.fileSuffixNum, mgr.oldestFileChunkSuffixNum)
		err := mgr.dataLoad(lp)
		if err == nil {
			defer mgr.rmBlockFile(lp.fileSuffixNum)
		}
	}

	if mgr.conf.dumpConf.Enabled {
		mgr.rwMutexs[lp.fileSuffixNum].RLock()
		defer mgr.rwMutexs[lp.fileSuffixNum].RUnlock()
	}
...
func (mgr *blockfileMgr) fetchRawBytesWithRetry(lp *fileLocPointer) ([]byte, error) {
	var result []byte
	var err error
	retryTimes := 1
	if mgr.conf.dumpConf.Enabled && mgr.conf.dumpConf.LoadRetryTimes > 1 {
	...
	for i := 0; i < retryTimes; i++ {
		if result, err = mgr.fetchRawBytes(lp); err == nil {
			return result, err
		}
	}
	return nil, fmt.Errorf("FetchRawBytesError: [%s]", err)
}
...
func (mgr *blockfileMgr) tryDataDump(dumpReason int) error {
	...
	case datadump.DumpForCronTab:
		logger.Debug("BlockFile DataDump: cronTab")
		return mgr.dataDump()
	case datadump.DumpForBlockFileLimit:
		fileLimit := mgr.conf.dumpConf.MaxFileLimit
		nowFileCount := mgr.cpInfo.latestFileChunkSuffixNum - mgr.oldestFileChunkSuffixNum + 1
		if nowFileCount > fileLimit {
			logger.Debugf("BlockFile DataDump: fileLimit:[%d],nowFileCount:[%d]", fileLimit, nowFileCount)
			return mgr.dataDump()
...
func (mgr *blockfileMgr) dataDump() error {
	mgr.dumpMutex.Lock()
	defer mgr.dumpMutex.Unlock()

	channel := filepath.Base(mgr.rootDir)

	var blockFiles []string
	startFileSuffixNum := mgr.oldestFileChunkSuffixNum
	endFileChunkSuffixNum := mgr.oldestFileChunkSuffixNum - 1

	for idx := startFileSuffixNum; idx <= mgr.cpInfo.latestFileChunkSuffixNum-1; idx++ {
		orgPath := deriveBlockfilePath(mgr.rootDir, idx)
		if ok, _, _ := util.FileExists(orgPath); ok {
			if fileTime, err := util.FileModTime(orgPath); err == nil {
				if time.Now().Sub(fileTime) > mgr.conf.dumpConf.DumpInterval {
					blockFiles = append(blockFiles, orgPath)
					endFileChunkSuffixNum = idx
				} else {
					break
				}
			}
	...
	if len(blockFiles) <= 0 {
		logger.Debugf("No files to dump")
		return nil
	} else {
		var number int
		if _, err := fmt.Sscanf(blockFiles[0], filepath.Join(mgr.rootDir, blockFileFormatString), &number); err != nil {
			logger.Debugf("Failed to get startFileSuffixNum: [%s]", err)
		} else {
			startFileSuffixNum = number
		}
	}

	dumpInfo := &blockDumpInfo{
		StartBlockNum: 0,
		EndBlockNum:   mgr.cpInfo.fileEndBlockNums[endFileChunkSuffixNum],
	}
	if startFileSuffixNum > 0 {
		dumpInfo.StartBlockNum = mgr.cpInfo.fileEndBlockNums[startFileSuffixNum-1] + 1
	}
	writeDumpIndex(mgr.rootDir+"/index", dumpInfo)
	blockFiles = append(blockFiles, mgr.rootDir+"/index")

	tarName := fmt.Sprintf(tarFileFormatString, startFileSuffixNum, endFileChunkSuffixNum)
	if err := util.TarFiles(blockFiles, filepath.Join(mgr.conf.dumpConf.DumpDir, channel, tarName)); err != nil {
		return fmt.Errorf("Cannot tar the blockfiles for %s", err)
	}

	mgr.oldestFileChunkSuffixNum = endFileChunkSuffixNum + 1
	//归档结束后，删除blockfile
	for idx := startFileSuffixNum; idx <= endFileChunkSuffixNum; idx++ {
		mgr.rmBlockFile(idx)
	}
	os.Remove(mgr.rootDir + "/index")
	return nil
}

func (mgr *blockfileMgr) dataLoad(lp *fileLocPointer) error {
	mgr.rwMutexs[lp.fileSuffixNum].Lock()
	defer mgr.rwMutexs[lp.fileSuffixNum].Unlock()
	file := deriveBlockfilePath(mgr.rootDir, lp.fileSuffixNum)
	for ok, _, _ := util.FileExists(file); !ok; ok, _, _ = util.FileExists(file) {
		channel := filepath.Base(mgr.rootDir)
		srcDir := filepath.Join(mgr.conf.dumpConf.LoadDir, channel)
		fileName := blockfilePrefix + fmt.Sprintf("%06d", lp.fileSuffixNum)
		return util.LoadFileByNumber(srcDir, mgr.rootDir, fileName, uint64(lp.fileSuffixNum), tarFileFormatString)
	}
	return nil
}

func (mgr *blockfileMgr) rmBlockFile(fileNum int) error {
	mgr.rwMutexs[fileNum].Lock()
	defer mgr.rwMutexs[fileNum].Unlock()
	filename := deriveBlockfilePath(mgr.rootDir, fileNum)
	return os.Remove(filename)
}

func (mgr *blockfileMgr) writeConfigBlock(block *common.Block) error {
	blockBytes, _, err := serializeBlock(block)
	if err != nil {
		return fmt.Errorf("Error while serializing block: %s", err)
	}
	return ioutil.WriteFile(filepath.Join(mgr.rootDir, configBlockFileName), blockBytes, 0666)
}

func (mgr *blockfileMgr) readConfigBlock() (*common.Block, error) {
	cfgBlockFile := filepath.Join(mgr.rootDir, configBlockFileName)
	if blockBytes, err := ioutil.ReadFile(cfgBlockFile); err != nil {
		return nil, err
	} else {
...
func (i *checkpointInfo) marshal() ([]byte, error) {
	...
	for idx := 0; idx < i.latestFileChunkSuffixNum; idx++ {
		if err = buffer.EncodeVarint(i.fileEndBlockNums[idx]); err != nil {
			return nil, err
		}
	}
...
func (i *checkpointInfo) unmarshal(b []byte) error {
	...
	i.fileEndBlockNums = make([]uint64, i.latestFileChunkSuffixNum)
	for idx := 0; idx < i.latestFileChunkSuffixNum; idx++ {
		if val, err = buffer.DecodeVarint(); err != nil {
			return err
		}
		i.fileEndBlockNums[idx] = val
	}
...
func (i *checkpointInfo) String() string {
	endBlockNumStr := "["
	for _, blockNum := range i.fileEndBlockNums {
		endBlockNumStr += fmt.Sprintf("%d,", blockNum)
	}
	endBlockNumStr += "]"
	return fmt.Sprintf("latestFileChunkSuffixNum=[%d], latestFileChunksize=[%d], isChainEmpty=[%t], lastBlockNumber=[%d],fileEndBlockNums=[%s]",
		i.latestFileChunkSuffixNum, i.latestFileChunksize, i.isChainEmpty, i.lastBlockNumber, endBlockNumStr)
