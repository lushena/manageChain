...
func (h *Handler) deliverBlocks(ctx context.Context, srv *Server, envelope *cb.Envelope) error {
	...
	if err := accessControl.Evaluate(); err != nil {
		...
		// when orderer requests application chain's data, it will pass through successfully, used when handling raft snapshots
		logger.Debug("Try evaluating authorization for system chain, it is used when pulling data of application chain from other orderers when handling raft snapshot")
		systemChainID := h.ChainManager.SystemChannelID()
		systemChain, ok := h.ChainManager.GetChain(systemChainID)
		if !ok {
			// it happens in peer deliver, just return forbidden
			return srv.SendStatusResponse(cb.Status_FORBIDDEN)
		}
		systemControl, err := NewSessionAC(systemChain, envelope, srv.PolicyChecker, systemChainID, crypto.ExpiresAt)
		if err != nil {
			logger.Warningf("[channel: %s] failed to create access control object due to %s", systemChainID, err)
			return srv.SendStatusResponse(cb.Status_BAD_REQUEST)
		}
		if err := systemControl.Evaluate(); err != nil {
			logger.Warningf("[channel: %s] Client authorization revoked for deliver request from %s: %s", systemChainID, addr, err)
			return srv.SendStatusResponse(cb.Status_FORBIDDEN)
		}
	}
